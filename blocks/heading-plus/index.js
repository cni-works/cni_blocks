( function( blocks, element, blockEditor, components, data, i18n, richText ) {
	...
	function customDesignEditorCss( design ) {
		...
		const tabletBase = typeof design.tablet_base_css === 'string' ? design.tablet_base_css.trim() : '';
		const tabletBefore = typeof design.tablet_before_css === 'string' ? design.tablet_before_css.trim() : '';
		const tabletAfter = typeof design.tablet_after_css === 'string' ? design.tablet_after_css.trim() : '';
		const mobileBase = typeof design.mobile_base_css === 'string' ? design.mobile_base_css.trim() : '';
		const mobileBefore = typeof design.mobile_before_css === 'string' ? design.mobile_before_css.trim() : '';
		const mobileAfter = typeof design.mobile_after_css === 'string' ? design.mobile_after_css.trim() : '';
		const hasPseudo = before || after || tabletBefore || tabletAfter || mobileBefore || mobileAfter;
		const positionedBase = hasPseudo && ! /position\s*:/.test( base ) ? 'position:relative;display:inline-block;' + base : base;
		const advanced = typeof design.css === 'string' ? design.css.replace( /&/g, scope ) : '';
		const deviceRules = function( target, ownBase, ownBefore, ownAfter ) { return ( ownBase ? target + '{' + ownBase + '}' : '' ) + ( ownBefore ? target + '::before{' + ownBefore + '}' : '' ) + ( ownAfter ? target + '::after{' + ownAfter + '}' : '' ); };
	...
	function designPreview( design, text, style ) {
		...
		/* The generic structural selector already supplies the underline and
		 * notch to cards. Adding a preview-only line here would draw it twice. */
		if ( design.category === 'heading' && design.id === 'speech-underline' ) return el( 'span', { className: 'cni-heading-plus__design-sample cni-heading-plus__effect-target', style: style }, previewText );
	...
	function styleFor( a ) {
		...
		if ( headingDesign && [ 'left-line', 'right-line' ].indexOf( headingDesign.id ) !== -1 ) {
			style[ '--cni-heading-structure-line-length' ] = Math.max( .5, Math.min( 12, numberOr( a.headingDesignLineLength, 2.8 ) ) ) + 'em';
			style[ '--cni-heading-structure-line-gap' ] = Math.max( 0, Math.min( 4, numberOr( a.headingDesignLineGap, .45 ) ) ) + 'em';
			style[ '--cni-heading-structure-line-color' ] = a.headingDesignLineColor || a.headingDesignAccentColor || headingDesign.accentColor;
		}
	...
	function marginControls( props, attribute, label, icon ) {
		const defaults = headingDefaultMargins( textTag( props.attributes ) ).pc;
		const defaultValue = attribute === 'marginTop' ? defaults[ 0 ] : defaults[ 1 ];
		const presets = [ [ __( '標準', 'cni-blocks' ), undefined, defaultValue ], [ '0', 0, 0 ], [ 'XXS', 4, 4 ], [ 'XS', 8, 8 ], [ 'S', 16, 16 ], [ 'M', 24, 24 ], [ 'L', 32, 32 ], [ 'XL', 40, 40 ], [ 'XXL', 56, 56 ] ];
		const currentValue = typeof props.attributes[ attribute ] === 'number' ? props.attributes[ attribute ] : undefined;
		const current = presets.find( function( preset ) { return currentValue === preset[ 1 ]; } ) || presets[ 0 ];
		const direction = attribute === 'marginTop' ? '↑' : '↓';
		return el( ToolbarDropdownMenu, { icon: el( 'span', { className: 'cni-heading-plus__margin-toolbar-label', 'aria-hidden': 'true' }, direction + current[ 0 ] ), label: label + '：' + current[ 0 ] + '（' + current[ 2 ] + 'px）', controls: presets.map( function( preset ) { const isCurrent = currentValue === preset[ 1 ]; return { title: label + '：' + preset[ 0 ] + '（' + preset[ 2 ] + 'px）', icon: isCurrent ? 'yes' : undefined, isActive: isCurrent, onClick: function() { setMarginPreset( props, attribute, preset[ 1 ] ); } }; } ) } );
	}
			...
			return el( element.Fragment, null,
				...
				el( InspectorControls, null,
					el( PanelBody, { title: __( '見出しデザイン・文字装飾', 'cni-blocks' ), initialOpen: true },
						...
						activeHeadingDesign && [ 'left-line', 'right-line' ].indexOf( activeHeadingDesign.id ) !== -1 ? el( element.Fragment, null,
							palette( __( 'ラインの色', 'cni-blocks' ), a.headingDesignLineColor || a.headingDesignAccentColor || activeHeadingDesign.accentColor, function( value ) { props.setAttributes( { headingDesignLineColor: value || '' } ); } ),
							el( RangeControl, { label: __( 'ラインの長さ（文字サイズ比）', 'cni-blocks' ), value: numberOr( a.headingDesignLineLength, 2.8 ), min: .5, max: 12, step: .1, onChange: function( value ) { props.setAttributes( { headingDesignLineLength: value } ); } } ),
							el( RangeControl, { label: __( '文字との間隔（文字サイズ比）', 'cni-blocks' ), value: numberOr( a.headingDesignLineGap, .45 ), min: 0, max: 4, step: .05, onChange: function( value ) { props.setAttributes( { headingDesignLineGap: value } ); } } )
						) : null,
